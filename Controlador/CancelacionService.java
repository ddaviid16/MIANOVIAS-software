        ...
        try {
            ...
            try (PreparedStatement ps = cn.prepareStatement(
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (!rs.next())
                        throw new SQLException("Nota no encontrada: " + numeroNota);
            ...
            //    (solo renglones que todavía no estaban en 'C')
            try (PreparedStatement ps = cn.prepareStatement(
            ...
            // 3) (Opcional) Reponer existencias de OBSEQUIOS
            try (PreparedStatement ps = cn.prepareStatement(
                ...
                // Si aún no tienes las columnas *_cod, se ignora y no revienta la cancelación.
            }

            // 4) Encabezado de la nota -> status='C'
            try (PreparedStatement up = cn.prepareStatement(
            ...
            // 5) Tablas relacionadas: SOLO ACTUALIZAR STATUS, NADA DE DELETE
            //    Nota_Detalle: aquí es donde LOGRAMOS lo que quieres:
            //    el detalle sigue existiendo, solo queda marcado como 'C'.
            execUpdateSafe(cn,
                    "UPDATE Nota_Detalle SET status='C' " +
                    "WHERE numero_nota=? AND status <> 'C'",
                    numeroNota);

            execUpdateSafe(cn,
                    "UPDATE Formas_Pago SET status='C' " +
                    "WHERE numero_nota=? AND status <> 'C'",
                    numeroNota);

            execUpdateSafe(cn,
                    "UPDATE Obsequios SET status='C' " +
                    "WHERE numero_nota=? AND status <> 'C'",
                    numeroNota);

            execUpdateSafe(cn,
                    "UPDATE Cambios_Fecha_Evento SET status='C' " +
                    "WHERE numero_nota=? AND status <> 'C'",
                    numeroNota);

            execUpdateSafe(cn,
                    "UPDATE Cancelacion_Eventos SET status='C' " +
                    "WHERE numero_nota=? AND status <> 'C'",
                    numeroNota);

            // 6) Bitácora de cancelación (upsert en Cancelados)
