...
public class DetalleClienteReportePanel extends JPanel {
    ...
    private void buscar() {
        ...
        try {
            ...
            if (list.isEmpty()) {
                ...
                // Selecciona la primera para que cargue el detalle de inmediato
    ...
    /** Carga productos y fechas agregadas de la nota actualmente seleccionada. */
private void cargarDetalleYFechasDeNotaSeleccionada() {
    ...
    // NUEVO: leer el TIPO de la nota (col 1 del MODELO), aunque #Nota esté oculto en la vista
    ...
    try {
        ...
        // NUEVO: si es ABONO, mostrar a qué crédito abona (como en NotasPorMes)
        if ("AB".equals(tipo)) {
        String folioCR = obtenerFolioCreditoAbonado(numeroNota);
        folioCR = (folioCR == null) ? "" : folioCR.trim();
        modelFechasNota.addRow(new Object[]{"Abona a (folio)", folioCR.isEmpty() ? "—" : folioCR});
    }
        ...
        JOptionPane.showMessageDialog(this, "Error cargando detalle/fechas: " + ex.getMessage(),
                "Error", JOptionPane.ERROR_MESSAGE);
    }
}
...
private void exportarCSVCliente() {
    ...
    try {
        // ============ (a) CSV de INFO (lo que ya tienes cargado en tbInfo) ============
        ...
        // ============ (b) CSV de DETALLE (notas + renglones + pagos + flags fechas) ============
        ...
        for (NotasDAO.NotaResumen r : notas) {
            ...
            if ("AB".equalsIgnoreCase(tipo)) {
                folioCreditoAbonado = obtenerFolioCreditoAbonado(numero);
            }

            // Detalle renglones (incluye PEDIDOS vía tu listarDetalleDeNota)
            ...
            String eventoNota   = formateaConMixto(ev);  // "—", "yyyy-MM-dd" o "Mixtas (n)"
            ...
            if (dets.isEmpty()) {
                // fila “vacía” si no hubo renglones (p.ej., errores o notas sin detalle)
                CsvDetalle row = new CsvDetalle();
                row.telefono = tel; row.nombre = nombre;
                row.numeroNota = numero; row.folio = folio; row.fecha = fecha; row.tipo = tipo; row.status = status;
                row.total = total; row.saldo = saldo;
                row.p_efectivo=pef; row.p_tcredito=pcr; row.p_tdebito=pdb; row.p_amex=pam;
                row.p_transfer=ptr; row.p_deposito=pdep; row.p_devolucion=pdv; row.p_ref_dv=refDV;
                row.folioCreditoAbonado = folioCreditoAbonado;
                row.eventoNota = eventoNota; row.entregaNota = entregaNota;
