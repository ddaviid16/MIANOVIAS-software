...
public class DevolucionPanel extends JPanel {
    ...
    public DevolucionPanel() {
        ...
        // Panel con teléfono + botón buscar
JButton btBuscarNombre = new JButton("Buscar por nombre");
JPanel pnlTel = new JPanel(new BorderLayout(5, 0));
pnlTel.add(txtTelefono, BorderLayout.CENTER);
pnlTel.add(btBuscarNombre, BorderLayout.EAST);

addCell(top,c,1,y,pnlTel,1,true);
...
/** Abre un diálogo para buscar cliente por nombre/apellidos y carga su teléfono. */
private void seleccionarClientePorApellido() {
    ...
    if (cr != null) {
        ...
        if (tel != null && !tel.isEmpty()) {
            // Poner teléfono → el DocumentListener ya llama a cargarClienteYNotas()
